...
class SAM3MC_DINO(nn.Module):
    def __init__(self, cfg):
        ...
        self.test_text_classifier2 = None
    ...
    def get_fcclip_text_classifier(self, dataname):
        if self.training:
            if self.train_dataname != dataname:
                ...
                text_classifier /= text_classifier.norm(dim=-1, keepdim=True)
                self.train_text_classifier2 = text_classifier
                ...
                # exit()
            return self.train_text_classifier2, self.train_num_templates
        else:
            if self.test_dataname != dataname or self.test_text_classifier2 is None:
                ...
                text_classifier = text_classifier.reshape(text_classifier.shape[0]//len(VILD_PROMPT), len(VILD_PROMPT), text_classifier.shape[-1]).mean(1) 
                text_classifier /= text_classifier.norm(dim=-1, keepdim=True)
                self.test_text_classifier2 = text_classifier
                self.test_dataname = dataname
                self.class_name_of_classifier = [element for index, element in enumerate(self.train_class_names) if index % len(VILD_PROMPT) == 0]
            return self.test_text_classifier2, self.test_num_templates
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            queries_seg_result = torch.einsum("bnc,bnhw->bchw", query_cls_results, queries_masks) # [bs, num_classes, H, W]
            
            #使用DINO做maskpool
            text_classifier2, _ = self.get_fcclip_text_classifier(meta['dataname'])
            ...
            # print("img_feat_for_pool shape:", img_feat_for_pool.shape) # bs, D, H', W'
            # print("queryes_masks shape:", queries_masks.shape) # bs, N, H, W
            ...
            # 执行 Pooling
            # 假设 self.mask_pooling 内部是 einsum / area
            # 如果 self.mask_pooling 内部没有除以面积，需要手动处理防止数值爆炸
            pooled_img_feat = self.mask_pooling(img_feat_for_pool, binarized_mask)
            pooled_img_feat = F.normalize(pooled_img_feat, dim=-1, p=2)

            # pooled_img_feat = self.mask_pooling(img_feat_for_pool, mask_for_pool)
            # print("pooled_img_feat shape:", pooled_img_feat.shape)
            logit_scale = torch.clamp(self.logit_scale.exp(), max=30.0)
            # print("text_classifier2 shape:", text_classifier2.shape) # num_names, dim
            out_vocab_cls_results = get_classification_logits(pooled_img_feat, text_classifier2, self.logit_scale, num_templates)[...,:-1]
            out_vocab_cls_results = out_vocab_cls_results.softmax(-1)

            in_vocab_cls_results = query_cls_results
            ...
            queries_seg_result = torch.einsum("bnc,bnhw->bchw", cls_results, mask_pred_results) # [bs, num_classes, H, W]
