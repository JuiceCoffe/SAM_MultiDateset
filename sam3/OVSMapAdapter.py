...
class OVSMapAdapter(nn.Module):
        ...
        if self.use_attnpool:
            if self.new_pool_decoder:
                ...
                box_matcher = BOXHungarianMatcher(
                    cost_class=cfg.MODEL.ATTNPOOL.CLASS_WEIGHT * 0,
                    cost_bbox=bbox_weight,
                    cost_giou=giou_weight,
                )
    ...
    def _freeze(self):
        ...
        for name, param in self.named_parameters():
            ...
            if match_freeze and match_train:
                # 冲突情况：同时命中，强制保证可学习并记录日志 (使用 warning 可以让这条日志更醒目)
    ...
    def forward(self, batched_inputs):
        ...
        if USE_GT_NAMES_ONLY:
            ...
            language_features_input = torch.cat(language_features_input, dim=0)
            language_mask_input = [self.language_mask[batch_gt_names_idx[i], :] for i in range(bs)]
            language_mask_input = torch.cat(language_mask_input, dim=0)
            if bs == 1:
